...
class Solution {
    ...
    private getSide(row: number, col: number, source: Direction, sides: Boolean[]): number{
        const current = this.map[row][col];
        const up = row > 0 ? this.map[row - 1][col] : null;
        const right = col < this.maxCol ? this.map[row][col + 1] : null;
        const down = row < this.maxRow ? this.map[row + 1][col] : null;
        const left = col > 0 ? this.map[row][col - 1] : null;
        // source UP
        // compare sides from up and 
        const upSide = current !== up && !sides[0];
        const rightSide = current !== right && !sides[1];
        const downSide = current !== down && !sides[2];
        const leftSide = current !== left && !sides[3];

        const originalSidesCount = sides.filter((item) => item === true).length;

        if (source === Direction.UP) {
            this.perimeter += Number(rightSide) + Number(downSide) + Number(leftSide);
        } else if (source === Direction.RIGHT) {
            this.perimeter += Number(upSide) + Number(downSide) + Number(leftSide);
        } else if (source === Direction.DOWN) {
            this.perimeter += Number(upSide) + Number(rightSide) + Number(leftSide);
        } else if (source === Direction.LEFT) {
            this.perimeter += Number(upSide) + Number(downSide) + Number(rightSide);
        }
        
        // add side when they meet side different from previous 
        return 0;
    }
